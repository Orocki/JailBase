...
public class HelloController implements Initializable {
    ...
    @FXML
    private TextArea state_text, jail_text;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        cb_stateauswahl.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            ...
            try {
                BufferedReader in = new BufferedReader(new FileReader("C:\\Users\\Donik\\Downloads\\IDs\\" + newValue +".txt" ));
        ...
        cb_jailauswahl.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
                    jail_text.setText(newValue);
        });

        btn_inmatesanzeigen.setOnAction(event -> {
            String jailauswahl = cb_jailauswahl.getSelectionModel().getSelectedItem();
            JailAuswahl.setAuswahl(jailauswahl);
    ...
    private void openNewView() {
        try {
            ...
            Scene scene2 = new Scene(fxmlLoader.load(), 1420,989 );
